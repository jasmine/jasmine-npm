...
class ParallelRunner extends RunnerBase {
  ...
  async execute2_(files, filterString) {
    ...
    const keepalive = setInterval(function() {}, 100000);
    ...
    const jasmineDoneInfo = await this.reportJasmineDone_();
  ...
  configureEnv_(envConfig) {
    // TODO: fail if config contains unsupportable properties like random:false,
    // seed, etc.
