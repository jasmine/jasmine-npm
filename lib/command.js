...
function Command(projectBaseDir, examplesDir, deps) {
  const {print, jasmine, parallelRunner} = deps;
  ...
  this.run = async function(args) {
    ...
    if (commandToRun) {
      ...
      if (options.unknownOptions.length > 0) {
        process.exitCode = 1;
        print('Unknown options: ' + options.unknownOptions.join(', '));
        print('');
        help({print: print});
      } else {
        await runJasmine(jasmine, parallelRunner, options);
      }
    }
  };
}
...
function parseOptions(argv) {
  ...
  for (const arg of argv) {
    ...
    } else if (arg.match("^--num-workers=(.*)")) {
      // TODO validate
      numWorkers = parseInt(arg.match("^--num-workers=(.*)")[1], 10);
    } else if (arg === '--') {
      break;
...
async function runJasmine(jasmine, parallelRunner, options) {
  let runner;

  if (options.numWorkers > 1) {
    runner = parallelRunner;
    runner.numWorkers = options.numWorkers;
  } else {
    runner = jasmine;
  }
