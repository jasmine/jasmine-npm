...
function parseOptions(argv, isWindows) {
  ...
  for (const arg of argv) {
    if (arg === '--no-color') {
      ...
      numWorkers = parseFloat(arg.match("^--num-workers=(.*)")[1]);
      if (isNaN(numWorkers) || numWorkers < 1 || numWorkers !== Math.floor(numWorkers)) {
        usageErrors.push('Argument to --num-workers= must be a positive integer');
      }
    } else if (arg === '--') {
      break;
...
function help(options) {
  ...
  print('Options:');
  print('%s\tRun in parallel with N workers', lPad('--num-workers=N', 18));
