...
function parseOptions(argv) {
  ...
  for (var i in argv) {
    ...
    } else if (arg.match("^--reporter-constructor=")) {
      reporterConstructor = arg.match("^--reporter-constructor=(.*)")[1];
    } else if (arg.match("^--reporter-configuration=")) {
      reporterConfiguration = arg.match("^--reporter-configuration=(.*)")[1];
    } else if (arg === '--') {
      break;
...
function help(options) {
  ...
  print('%s\treporter constructor field from reporter file exports if it is not the default export', lPad('--reporter-constructor=', 18));
  print('%s\treporter json configuration path to provide to the reporter constructor', lPad('--reporter-configuration=', 18));
  print('%s\tpath to reporter to use instead of the default Jasmine reporter', lPad('--reporter=', 18));
