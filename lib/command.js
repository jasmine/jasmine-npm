...
function parseOptions(argv, isWindows) {
  ...
  let usageErrors = [];
  let color = undefined;
  let listNotApplicableSpecs = undefined;
  let reporter;
  let configPath;
  let filterRegex;
  let filterPath;
  ...
  for (const arg of argv) {
    if (arg === '--no-color') {
      ...
      color = true;
    } else if (arg === '--list-not-applicable') {
...
async function runJasmine(Jasmine, ParallelRunner, projectBaseDir, options) {
  ...
  runner.configureDefaultReporter({
    color: options.color,
    listNotApplicableSpecs: options.listNotApplicableSpecs,
  });
...
function help(deps) {
  ...
  const options = [
    ...
    { syntax: '--list-not-applicable', help: 'list all specs that are marked not applicable' },
  ...
  for (const o of options) {
    print(wrapWithIndent(terminalColumns, lPad(o.syntax, 22) + '    ', o.help));
  }
