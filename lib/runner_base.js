...
class RunnerBase {
   ...
   * instance has already been configured a custom loader. Dynamic
  ...
  async loadConfig(config) {
    ...
    // TODO: Set up cross-package @see on the docs site and link this to the
    //  reporters config reference
    /**
     * Configuration for the default console reporter. See the documentation for
     * the @jasminejs/reporters package for details.
     * @name Configuration#defaultReporter
     * @type object | undefined
     */
    if (config.defaultReporter !== undefined) {
      this.configureDefaultReporter(config.defaultReporter);
    }

    /**
     * Whether the default reporter should list specs that are marked not
     * applicable rather than merely summarizing them.
     * @name Configuration#listNotApplicableSpecs
     * @type boolean | undefined
     * @default false
     */
    if (config.listNotApplicableSpecs !== undefined) {
      this.configureDefaultReporter({
        listNotApplicableSpecs: config.listNotApplicableSpecs
      });
    }

    /**
     * Specifies a module containing a custom loader that will be used to load
     * config files, source files, spec files, etc.
     *
     * If provided, it must refer to a module whose default export is a function
     * that loads modules. The function must take a single argument, which will
     * be a module import specifier or file path, and return a promise that
     * resolves to the corresponding loaded module. The argument will be a
     * module import specifier or file path.
     *
     ...
     */
    if (config.loader) {
      this.loader = { load: await this.loader.load(config.loader) };
    }
  ...
  configureDefaultReporter(options) {
    this.reporter_.configure(options);
  }
