...
class RunnerBase {
  ...
  loadConfig(config) {
     ...
     * @default false
     * @deprecated Use env.failSpecWithNoExpectations instead.
     */
    forwardToEnv('failSpecWithNoExpectations', false);
     ...
     * @type boolean | undefined
     * @default false
     * @deprecated Use env.stopSpecOnExpectationFailure instead.
     */
    forwardToEnv('stopSpecOnExpectationFailure', false);
     ...
     * @type boolean | undefined
     * @default false
     * @deprecated Use env.stopOnSpecFailure instead.
     */
    forwardToEnv('stopOnSpecFailure', false);
     ...
     * @type boolean | undefined
     * @default true
     * @deprecated Use env.random instead.
     */
    forwardToEnv('random', true);

    forwardToEnv('verboseDeprecations', false);

    function forwardToEnv(key, defaultValue) {
      if (config[key] !== undefined) {
        envConfig[key] = config[key];

        if (config[key] === defaultValue) {
          console.warn(
            `The ${key} configuration property is deprecated. Use env.${key} instead, or omit it since you're using the default setting.`
          );
        } else {
          console.warn(
            `The ${key} configuration property is deprecated. Use env.${key} instead.`
          );
        }
      }
    }
  ...
  configureDefaultReporter(options) {
    this.reporter_.configure({
      // TODO: only in derived non-parallel class
