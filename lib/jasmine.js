  ...
  return function (files) {
    ...
    const {includeFiles, excludeFiles} = files.reduce(function(ongoing, file) {
      ...
      if (!path.isAbsolute(file)) {
        // Don't use path.join because it will convert slashes to backslashes on Windows,
        // and glob interprets backslashes as escape sequences.
        file = [jasmineRunner.projectBaseDir, jasmineRunner.specDir, file].join('/');
     }
